...
- **提示词资源库**：汇总多个开源提示词仓库的图文精选内容，支持分类与标签单选联动筛选，并展示效果图、来源与许可信息。
...
首次启动或本地快照超过 24 小时时会优先从上游同步；如果刷新失败，会保留上次可用快照，首次准备失败时再尝试当前版本 GitHub Release 资源包。如需主动更新，在 `web/` 目录执行：

```bash
npm run sync:prompts
```

同步过程会记录上游效果图地址，但不会下载、裁切或二次处理图片；同时会校验来源数量、效果图覆盖率、重复正文、稳定 ID 和索引摘要。提示词库本地目录不会提交到 Git；GitHub Pages 每日重建最新快照，版本标签也会生成对应资源包。需要独立托管时可按开发文档仅将 Raw 索引发布到 Cloudinary。
