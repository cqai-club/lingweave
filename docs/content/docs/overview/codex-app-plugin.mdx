---
title: Codex App 插件
description: 在 Codex App 中安装并使用 LingWeave 插件
---

# Codex App 插件

LingWeave 提供了一个 Codex App 插件，用来把 Codex 和本地 Canvas Agent 连接起来。安装后，你可以直接在 Codex 中说“打开 LingWeave”，让 Codex 打开画布、读取节点、创建节点并触发生成流程。
...
确保 `~/.agents/plugins/marketplace.json` 中有 LingWeave 条目。这里的 `path` 要指向仓库里的插件子目录：
...
如果你正在 LingWeave 仓库中开发插件，可以直接使用仓库自带的 marketplace：
...
然后启动本地 Agent。端口不是 `3003` 时，把 `CANVAS_URL` 换成真实本地画布地址：

```bash
CANVAS_URL=http://localhost:3003 npx -y @basketikun/canvas-agent
```
