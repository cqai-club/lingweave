...
async function searchPrompts(input: SiteToolInput) {
    ...
    const result = await fetchPrompts({ keyword: String(input.keyword || ""), category: String(input.category || "全部"), model: String(input.model || "全部"), tag: tags, page, pageSize });
    return {
        total: result.total,
        page,
        pageSize,
        ...
        items: result.items.map((prompt) => ({
            ...
            sourceUrl: prompt.sourceUrl || prompt.githubUrl,
