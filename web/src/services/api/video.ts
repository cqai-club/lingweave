...
import { isAccountAiConfig, requestAiBlob, requestAiJson } from "@/services/api/account";
...
async function createOpenAIVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: RequestOptions): Promise<VideoGenerationTask> {
    ...
    try {
        const created = unwrapVideoResponse(await requestAiJson<ApiVideoResponse>(config, "/videos", { method: "POST", body, signal: options?.signal }));
...
async function pollOpenAIVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        const video = unwrapVideoResponse(await requestAiJson<ApiVideoResponse>(config, `/videos/${task.id}`, { signal: options?.signal }));
        const url = videoResultUrl(video);
        if (url) return { status: "completed", result: await videoResultFromUrl(url, options) };
        if (video.status === "completed") {
            const content = await requestAiBlob(config, `/videos/${task.id}/content`, { signal: options?.signal });
...
async function createSeedanceTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], videoReferences: ReferenceVideo[], audioReferences: ReferenceAudio[], options?: RequestOptions): Promise<VideoGenerationTask> {
    ...
    try {
        const created = unwrapSeedanceTask(
            await requestAiJson<ApiEnvelope<SeedanceTask>>(config, seedanceApiPath(), {
                method: "POST",
...
async function pollSeedanceTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        const state = unwrapSeedanceTask(await requestAiJson<ApiEnvelope<SeedanceTask>>(config, seedanceApiPath(task.id), { signal: options?.signal }));
...
function seedanceApiPath(taskId?: string) {
    return `/contents/generations/tasks${taskId ? `/${encodeURIComponent(taskId)}` : ""}`;
}
...
function assertVideoConfig(config: AiConfig, model: string) {
    ...
    if (!isAccountAiConfig(config) && !config.baseUrl.trim()) throw new Error("请先配置 Base URL");
    if (!isAccountAiConfig(config) && !config.apiKey.trim()) throw new Error("请先配置 API Key");
...
function readAxiosError(error: unknown, fallback: string) {
    ...
    if (axios.isAxiosError<{ error?: { message?: string }; msg?: string; message?: string; code?: number | string }>(error)) {
        if (!error.response && error.code === "ERR_NETWORK") return "请求被跨域拦截，请通过自己的服务转发请求或检查接口 CORS 配置";
